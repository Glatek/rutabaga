...
describe("post", () => {
	test("returns 402 if form data is malformed", async () => {
		...
		expect(response.statusText).toBe(
			`The provided form data did not validate against the schema. Reason: [{"instancePath":"","schemaPath":"#/required","keyword":"required","params":{"missingProperty":"firstName"},"message":"must have required property 'firstName'"}]`,
		);
	});
...
describe("get", () => {
	test("can get data", async () => {
		const getResponse = await rutabaga.api.get(
...
describe("patch", () => {
	test("returns 402 if form data is malformed", async () => {
		...
		expect(response.status).toBe(402);
		expect(response.statusText).toBe(
			`The provided form data did not validate against the schema. Reason: [{"instancePath":"","schemaPath":"#/required","keyword":"required","params":{"missingProperty":"age"},"message":"must have required property 'age'"}]`,
		);
	});
